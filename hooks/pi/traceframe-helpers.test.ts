 ...
 *
 * Run with: `bun test hooks/pi/traceframe-helpers.test.ts`
 *   or:    `node --test --import tsx/esm hooks/pi/traceframe-helpers.test.ts`
...
describe("sessionIDFromFile", () => {
	it("strips directory and extension from .jsonl files", () => {
		assert.equal(sessionIDFromFile("/u/m/.pi/agent/sessions/2026-07-03_abc.jsonl"), "2026-07-03_abc");
...
describe("sessionNameFromCwd", () => {
	...
	it("falls back to 'Pi session' when cwd is empty or missing", () => {
		assert.equal(sessionNameFromCwd(""), "Pi session");
		assert.equal(sessionNameFromCwd(undefined), "Pi session");
	});
});
