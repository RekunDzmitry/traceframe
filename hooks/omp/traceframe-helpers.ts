/**
 * Pure helpers for the Traceframe Omp extension.
 *
 * Anything that can be unit-tested in isolation lives here, with no
 * dependency on `fetch`, `process`, or the Omp extension API. The extension
 * entry point (`traceframe.ts`) imports these helpers and adds the
 * network/UI plumbing.
 *
 * Mirrors `hooks/pi/traceframe-helpers.ts` — the Omp extension uses the
 * same parsers, the same source denylist, and the same `lastAssistantText`
 * summary fallbacks. The only deltas are the `source: "omp"` tag and the
 * `"Omp session"` fallback label.
 */
