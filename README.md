...
The per-session view is the **timeline**: the chronological feed of
prompt → tool calls → assistant reply. Filter by kind/tool/failure/duration
or grep the file path. Click a row to expand input/output, "View raw" for the
full JSON payload.

Events are grouped by session, then by the user message that triggered them,
with three further levels of detail:

**User message group** — one collapsible group per message you typed,
holding every turn the agent produced before the next message. Collapsed by
default; the header shows the first line plus a turn and tool count. Because
the server closes a turn on every `Stop`, one message can contain several
turns. Two kinds of `UserPromptSubmit` are absorbed into the group before
them rather than heading their own: prompts injected by the host (background
agent reports, system reminders, `!` shell command output — matched on the
leading wrapper tag) and prompts superseded by a longer re-submission (a
strict prefix of the next one that ran no tools and arrived within 120s).
While a filter is active, groups holding a match are forced open.
